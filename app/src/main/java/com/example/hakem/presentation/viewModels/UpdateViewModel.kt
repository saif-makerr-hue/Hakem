package com.example.hakem.presentation.viewModels
...
import com.example.hakem.data.datasource.repository_imp.PatientRepositoryImp
import com.example.hakem.domain.model.PatientsItem
import com.example.hakem.presentation.components.UpdatePatient
...
class UpdateViewModel @Inject constructor(
    ...
    private fun changePatientInfo(change: (PatientsItem) -> PatientsItem) {
        _patientState.update { current ->
            when (current) {
                is PatientUiState.Success -> PatientUiState.Success(change(current.data))
                else -> current
            }
        }
    }
